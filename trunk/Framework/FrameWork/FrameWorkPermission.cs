    ...
    public class FrameWorkPermission : IHttpModule
    {
        ...
        private void app_Auth(object sender, EventArgs e)
        {
            ...
            if (UserOnlineList == null)
                UserOnlineList = new CacheOnline<string, OnlineUser<string>>();
            //进行在线更新检测
            ...
            if (Common.GetCharInStringCount("/manager/", Common.GetScriptUrl.ToLower()) > 0)
            {
                ...
                {
                    //清除在线用户
                    BusinessFacade.InterfaceClearMemberTimeOut();
                    ...
                    if (Common.Get_UserID != 0)
                    {
                        ...
                        if (BusinessFacade.InterfaceOnlineCheckKey(Common.GetSessionID))
                        {
                            BusinessFacade.InterfaceOnlineAccess(Common.GetSessionID);
                        }
                        else
                        {
