...
class CWindowFinder : public CWindowIterator 
{
public:
	enum FIND_TYPE{CAPTION, CLASS};
	enum MATCH_TYPE{EQUAL, INCLUDE};

	HWND m_hWnd;
	FIND_TYPE m_FindType;
	MATCH_TYPE m_MatchType;

	CWindowFinder(HWND hwnd, FIND_TYPE FindType=CAPTION, MATCH_TYPE MatchType=EQUAL);
	~CWindowFinder();

	HWND Find(CString str );
