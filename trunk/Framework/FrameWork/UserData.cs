...
namespace FrameWork
{
    ...
    public class UserData
    {
        ...
        /// 用户缓存
        /// </summary>
        public static IFrameWorkCache _UserCache = (IFrameWorkCache)Activator.CreateInstance(Common.GetCachenamespace, Common.GetCacheclassName).Unwrap();
        /// <summary>
        /// 根据用户ID获取用户资料
        /// </summary>
        ...
        public static sys_UserTable Get_sys_UserTable(int UserID)
        {
            ...
            {
                return (sys_UserTable)_UserCache[CacheKey];
            }
            else
            {
                sys_UserTable sUT = BusinessFacade.sys_UserDisp(UserID);
                _UserCache.Insert(CacheKey, sUT);
                return sUT;
            }
        }
        ...
        {
            _UserCache.Remove(string.Format("{0}-UserInfo-{1}", Common.Get_WebCacheName, UserID));
        }
        ...
        private static Hashtable Get_UserPermission(int UserID)
        {
            ...
            if (_UserCache[Key] != null)
            {
                return (Hashtable)_UserCache[Key];
            }
            else
            {

                Hashtable _Temp = Get_sys_RolePermissionTable(UserID);
                _UserCache.Insert(Key, _Temp);
                return _Temp;
            }
        }
        ...
        {
            _UserCache.Remove(string.Format("{1}-Permission-{0}", UserID, Common.Get_WebCacheName));
        }
